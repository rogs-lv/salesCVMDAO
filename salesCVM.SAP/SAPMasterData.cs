...
namespace salesCVM.SAP
{
    public class SAPMasterData : ISAPMasterData
    {
        ...
        public bool CreateItem(ref MensajesObj msjCreate, Models.SAP modelo, ItemData item, List<Propiedad> propiedades, List<Inventario> inventario) {
            string msj = string.Empty;
            Company _oCompany = null;
            try
            {
                if (isap.Conectar(ref msj, modelo))
                {
                    ...
                    oItem.ItemCode = item.ItemCode;
                    oItem.ItemName = item.ItemName;
                    oItem.ItemsGroupCode = item.ItmsGrpCod;
                    oItem.UoMGroupEntry = item.UgpEntry;
                    
                    oItem.SalesItem = (BoYesNoEnum)item.SellItem;
                    oItem.InventoryItem = (BoYesNoEnum)item.InvntItem;
                    oItem.PurchaseItem = (BoYesNoEnum)item.PrchseItem;
                    
                    oItem.Valid = (BoYesNoEnum)item.validFor;
                    
                    //Impuestos
                    oItem.WTLiable = (BoYesNoEnum)item.WTLiable;
                    oItem.VatLiable = (BoYesNoEnum)item.VATLiable;

                    //Propiedades
                    for(int p = 0; p < propiedades.Count; p++) {
                        if (propiedades[p].Status == true) {
                            oItem.Properties[propiedades[p].ItmsTypCod] = BoYesNoEnum.tYES;
                        }
                    }
                    //Inventario
        ...
        public bool UpdateItem(ref MensajesObj msjCreate, Models.SAP modelo, ItemData item, Propiedad propiedades, Inventario inventario) {
            return true;
        }
